...
import { questionAnswer, questionBody } from "../types";
...
describe('quizUpdate', () => {
    ...
    describe('Invalid Input Tests', () => {
      test('Question Id does not refer to a valid question within this quiz', () => {
        const res = requestQuizUpdate(quizId.quizId, questionId.questionId + 0.1, token.sessionId, questionBody);
      ...
      test('Question string is less than 5 characters in length', () => {
        ...
        const res = requestQuizUpdate(quizId.quizId, questionId.questionId, token.sessionId, questionBody);
        expect(res.statusCode).toBe(400);
        expect(res.body).toStrictEqual(ERROR);
      });
      ...
      test('Token is invalid', () => { 
        const res = requestQuizUpdate(quizId.quizId, questionId.questionId, token.sessionId + 'a', questionBody);
        expect(res.statusCode).toBe(401);
        expect(res.body).toStrictEqual(ERROR);
      });
