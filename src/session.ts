...
export const adminQuizSessionStart = (token: string, quizId: number, autoStartNum: number): SessionIdReturn | ErrorObject => {
  ...
  const numActiveSessions = data.sessions.filter(session => session.state !== sessionState.END).length; // can replace with adminQuizSessionView(token, quizId).activeSessions.length;
